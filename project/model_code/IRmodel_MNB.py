...
class IR_MNB(IRmodel):
    ...
    def get_model(self):
        clf = MultinomialNB()
        with open(os.path.join("model",self.data_name,"MNB.pkl") , 'wb') as f:
            pickle.dump(clf, f, pickle.HIGHEST_PROTOCOL)
        return clf

    def load_model(self):
        with open(os.path.join("model",self.data_name,"MNB.pkl") , 'rb') as f:
            clf = pickle.load(f)
        return clf
    ...
    def predict(self,X,raw_Y):
        ...
        f = open(os.path.join("result",self.data_name,"MNB.csv"),'w')
        wf = csv.writer(f)
